...
class ContigObj():
    ...
    def print_gc_by_position(self, window):
        start = 0
        stop = start + int(window/2) + 1
        pos = 0
        print "# %s %s %s" % (len(self.seq), self.header, self.seq)
        while stop < window:
        ...
        while stop <= len(self.seq):
            ...
            stop += 1
            pos += 1
        while start < (len(self.seq) - int(window/2)):
            subseq = self.seq[start:stop]
            print start, stop, subseq
            print "%s %s %s %s %s" % (self.header, pos, self.get_gc_score(subseq), len(self.seq), subseq)
            start += 1
    ...
    def print_gc_by_interval(self, window):
        start = 0
        stop = window
        subseq = ''
        print "# %s %s" % (len(self.seq), self.header)
        while start < len(self.seq):
            if stop > len(self.seq):
                stop = len(self.seq)
            subseq = self.seq[start:stop]
            print "%s %s %s %s" % (self.header, start, stop, self.get_gc_score(subseq))
            start += window
            stop += window
...
if __name__ == "__main__":
    ...
    for contig in sorted(contigs):
        contigObj = contigs[contig]
        contigObj.print_gc_by_interval(window)
